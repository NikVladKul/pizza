...
function addHandlersRollup() {
  ...
  headsOrders.forEach((elem) => elem.addEventListener("change", (event) => {
...
function renderTable(orders) {
  ...
  for (let i = 0; i < orders.length; i++) {
    ...
    for (let j = 0; j < orders[i].list.length; j++) {
      ...
      newCellList.innerHTML = orders[i].list[j].category + "/" + orders[i].list[j].name;
      ...
      newCellList.innerHTML = `<input type="checkbox" name=${"check" + orders[i].list[j].id} ${(orders[i].list[j].ready) ? "checked" : ""} data-id=${orders[i].list[j].id} data-orderid=${orders[i].id} data-field="ready">`;
...
async function onCheked(e) {
  const update = await updateDb(e.target.dataset.id, e.target.dataset.field, e.target.checked ? 1 : 0, "orders", e.target.dataset.orderid);
  if (update) {//заказ готов
    getOrders();//перерисовать

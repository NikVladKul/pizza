...
if (content)
  content.addEventListener("click", function (event) {
    getGoodId(event);
  }); // обработчик на содержимое
if (tableOrder)
  tableOrder.addEventListener("click", function (event) {
    getGoodId(event);
  }); // обработчик на содержимое
if (carousel)
  carousel.addEventListener("click", function (event) {
    getGoodId(event);
  }); // обработчик на карусель
popupGood.addEventListener("click", function (event) {
  getGoodId(event);
}); // обработчик на попап
...
if (logout) {
  ...
  if (localStorage.getItem("cart" + user.dataset.id)) {
    //Если что-то осталось в корзине, загружаем
    ...
    if (
      (event.target.tagName === "IMG" &&
        (event.currentTarget === content ||
          event.currentTarget === carousel)) ||
      event.target.tagName === "TD"
    ) {
      // Если клик по картинке открываем попап
      if (
...
function fillPopupGood(good) {
  ...
  for (let i = 0; i < elems.length; i++) {
    ...
    if (elems[i].classList.contains("description"))
      elems[i].innerHTML = good.description;
    if (elems[i].classList.contains("cost"))
      elems[i].textContent =
        "Цена: " +
        good.cost /*.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')*/ +
        "руб.";
    if (elems[i].classList.contains("img"))
      elems[i].setAttribute("src", good.img);
  }
}
